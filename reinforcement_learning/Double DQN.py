...
class Agent(object):
    ...
    def egreedy_action(self, state):
        ...
        with torch.no_grad():
            q_value = self.current_net(state_input).view(-1).cpu().numpy()
    ...
    def action(self, state):
        state_input = torch.Tensor(state).unsqueeze(0).to(DEVICE)
        with torch.no_grad():
            action_value = self.current_net(state_input).view(-1).cpu().numpy()
    ...
    def _train(self):
        ...
        max_action = torch.argmax(self.current_net(next_state_batch), -1)  # 在当前网络找出最大Q值对应的动作
        ...
        for i in range(0, BATCH_SIZE):
            ...
            if done:
                ...
                y_batch.append(samples[i].reward + GAMMA * next_q_value[i, max_action[i]])  # 选择动作对应的Q值
        y_batch = torch.Tensor(y_batch).to(DEVICE)
        q_value = self.current_net(state_batch)  # 当前Q网络选择选择动作

        self.optimizer.zero_grad()
        loss = self.criterion(torch.sum(q_value * action_batch, -1), y_batch)
        loss.backward()
        torch.nn.utils.clip_grad_norm_(self.current_net.parameters(), GRAD_CLIP)  # 更新当前Q网络的参数
        self.optimizer.step()
